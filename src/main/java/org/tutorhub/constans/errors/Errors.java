package org.tutorhub.constans.errors;
...
public enum Errors {
    ...
    SQL_INJECTION_WAS_INSERTED {
        @lombok.NonNull
        @org.jetbrains.annotations.Contract( value = "_, _ -> _" )
        public String translate (
                // значение введенное пользователем
                @lombok.NonNull final String inputParam,
                @lombok.NonNull final String sqlInjection
        ) {
            return MessageFormat.format(
                    "Value: {0} was found in sql injection list for prohibited value: {1}",
                    inputParam,
                    sqlInjection
            );
        }
